...
func HandleListPluginScripts(c *fiber.Ctx) error {
	scriptsDir := config.AppEnv.UserDataPath + "/Scripts"

	scriptFiles, err := utils.GetFilesRecursively(scriptsDir)
	if err != nil {
		zap.L().Error("Error getting plugin script files", zap.String("path", scriptsDir), zap.Error(err))
		return c.Status(fiber.StatusInternalServerError).SendString("Failed to get plugin script files")
	}
